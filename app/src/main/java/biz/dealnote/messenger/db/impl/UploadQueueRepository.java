 ...
 * phoenix
 */
class UploadQueueRepository extends AbsRepository implements IUploadQueueRepository {
    ...
    UploadQueueRepository(@NonNull AppRepositories base) {
        super(base);
    ...
    public Single<List<UploadObject>> getAll(Predicate<UploadObject> filter) {
        return Single.create(e -> {
            ...
            e.onSuccess(data);
            Exestime.log("UploadQueueRepository.getAll", start, "count: " + data.size() +
    ...
    public Single<List<UploadObject>> getByDestination(int accountId, int destId, int destOwnerId, @Method int method) {
        return Single.create(e -> {
            ...
            e.onSuccess(data);
            Exestime.log("UploadQueueRepository.getByDestination", start, "count: " + data.size() +
                    ", ids: " + Utils.join(data, ",", orig -> String.valueOf(orig.getId())));
        });
    }
    ...
    public Single<List<UploadObject>> put(List<UploadIntent> intents) {
        return Single.create(e -> {
            ...
            Exestime.log("UploadQueueRepository.put", start, "count: " + intents.size() +
    ...
    public Completable removeWithId(final int id, final BaseUploadResponse response) {
        return Completable.create(emitter -> {
            ...
            if(count > 0){
                ...
                Exestime.log("UploadQueueRepository.removeById", start, "id: " + id + ", hasResponse: " + nonNull(response));
            } else {
    ...
    public Maybe<UploadObject> findFirstByStatus(int status) {
        return Maybe.create(emitter -> {
            ...
            Exestime.log("UploadQueueRepository.findFirst", start, "object: " + object);
        });
    }
    ...
    public Completable changeStatus(int id, int status) {
        return Completable.create(emitter -> {
            ...
            if (count > 0) {
                ...
                Exestime.log("UploadQueueRepository.changeStatus", start, "id: " + id + ", status:" + statusOf(status));
            } else {
                emitter.onError(new DatabaseException("Record with id " + id + " not found"));
            }
        });
