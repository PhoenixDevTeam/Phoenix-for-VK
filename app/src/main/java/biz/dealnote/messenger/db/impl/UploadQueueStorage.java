...
class UploadQueueStorage extends AbsStorage implements IUploadQueueStorage {
    ...
    UploadQueueStorage(@NonNull AppStorages base) {
        super(base);
    ...
    public Single<List<UploadObject>> getAll(Predicate<UploadObject> filter) {
        return Single.create(e -> {
            ...
            e.onSuccess(data);
            Exestime.log("UploadQueueStorage.getAll", start, "count: " + data.size() +
    ...
    public Single<List<UploadObject>> getByDestination(int accountId, int destId, int destOwnerId, @Method int method) {
        return Single.create(e -> {
            ...
            e.onSuccess(data);
            Exestime.log("UploadQueueStorage.getByDestination", start, "count: " + data.size() +
                    ", ids: " + Utils.join(data, ",", orig -> String.valueOf(orig.getId())));
        });
    }
    ...
    public Single<List<UploadObject>> put(List<UploadIntent> intents) {
        return Single.create(e -> {
            ...
            Exestime.log("UploadQueueStorage.put", start, "count: " + intents.size() +
    ...
    public Completable removeWithId(final int id, final BaseUploadResponse response) {
        return Completable.create(emitter -> {
            ...
            if(count > 0){
                ...
                Exestime.log("UploadQueueStorage.removeById", start, "id: " + id + ", hasResponse: " + nonNull(response));
            } else {
    ...
    public Single<Optional<UploadObject>> findFirstByStatus(int status) {
        return Single.create(emitter -> {
            ...
            Exestime.log("UploadQueueStorage.findFirst", start, "object: " + object);
        });
    }
    ...
    public Completable changeStatus(int id, int status) {
        return Completable.create(emitter -> {
            ...
            if (count > 0) {
                ...
                Exestime.log("UploadQueueStorage.changeStatus", start, "id: " + id + ", status:" + statusOf(status));
            } else {
                emitter.onError(new DatabaseException("Record with id " + id + " not found"));
            }
        });
